...
1. tidyverse
2. lubridate (a package to convert text to dates and back)
...
This is the URL for the opioid emergency data we used in class a while back. To read it, we're using a package called *readr*, and the read_csv() command. This avoids some of the problems with recognizing data types that other importers have in R. 
...
## Copying and renaming to a new data frame
...
3. **Select** the columns you want, and rename them at the same time. You may have to put some of the old names in quotation marks if they have special characters or spaces in them. 

<p style="background-color:#e6f0ff;padding:10px;"> R is **case-sensitive**, meaning that *age* is not the same thing as *Age*. Use an = sign to connect the new name (on the left) to the old name (on the right). Leave out the new name and the = sign if you want to use the old name. </p>

You might note that I try to space out my commands and add whitespace where possible. The indentation is used to remind me what parts of the commands are connected to one another. 
...
Filtering is done in a much more sophisticated way in R. 
...
opioid_calls %>%
  ...
  head(50) #just show the first 50 rows to keep your browser from choking
...
opioid_calls %>%
  ...
  head()  #again, to avoid choking the browser. 
...
In R, grouping and counting are done in different steps. (There's a reason for this - it allows you to create subtotals, but that's too confusing for now).  Think if group_by and summarise (British spelling) as a pivot table row element and the COUNT in the data section. In this case, we're grouping by *dow* (day of week), then counting the number of cases , using n(). 

At the same time, we want to sort the output by the highest number of cases to the lowest, which is done using the *arrange* command.
